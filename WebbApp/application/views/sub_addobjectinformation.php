...
echo "<div id='sub'>". form_open_multipart('index.php/site/add_object_information/');
	...
	//<img src='".base_url()."styles/images/rotateplus.png' alt=''/>
		...
		<option value='".htmlentities($row -> size_x)."'> ".htmlentities($row -> size_x)." </option>
		...
		<label>Storlek Y:</label>
		<select type='text' name='size_y".$column."' value='".htmlentities($row -> size_y)."' >  
		<optgroup label='Vald'></optgroup>
		<option value='".htmlentities($row -> size_y)."'> ".htmlentities($row -> size_y)." </option>
		<optgroup label='----------'></optgroup>
		<option value='1'>1 </option>
		<option value='2'> 2 </option>
		<option value='3'> 3 </option>
