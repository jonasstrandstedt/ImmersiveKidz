...
void HUD::keyboardButton(int key,int state, std::vector<Illustration*> illu) 
{
	...
	if(_zoom <= 1)
		if((key == GLFW_KEY_KP_SUBTRACT || key == '-') && state == GLFW_PRESS) _zoom = _zoom + static_cast<float>(0.1);
	if(_zoom > 0.11)
		if((key == GLFW_KEY_KP_ADD  || key == '+') && state == GLFW_PRESS) _zoom = _zoom - static_cast<float>(0.1);
	if(_zoom > 1)
		_zoom = 1;
	if(_zoom < 0.11)
		_zoom = 0.11;
